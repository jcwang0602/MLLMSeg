...
class PackedDataset(IterableDataset):
    def __init__(
        ...
        if get_rank() == 0:
            logger.info(
                f"Loaded dataset to pack: {ds_name_list}, {self.num_images_expected=}, {self.max_packed_tokens=}, {self.replacement=}, {self.allow_overflow=}",
            )
    ...
    def update_buffer_list(self, buffer_list, buffer_max_len_list, buffer):
        ...
        for each_buffer in splitted_buffer:
            if each_buffer["pixel_values"].size(0) > self.num_images_expected:
                logger.error(f"Find a sample with {each_buffer['pixel_values'].size(0)} images, which exceeds {self.num_images_expected}")
                continue
        ...
        while True:
            ...
            while len(buffer_max_len_list) > 0:
                if buffer_max_len_list[0]["pixel_values"].size(0) != self.max_packed_tokens:
                    logger.debug(f"num tokens of a buffer exceed {self.max_packed_tokens=}, yield a sample with {buffer_max_len_list[0]['pixel_values'].size(0)} images")
            ...
            while len(buffer_list) > 0 and buffer_list[0]["pixel_values"].size(0) > self.num_images_expected:
                logger.error(f"num images of a buffer ({buffer_list[0]['pixel_values'].size(0)}) is larger than num_images_expected({self.num_images_expected})")
                buffer_list.pop(0)
            ...
            while len(buffer_list) > self.max_buffer_size:
                logger.debug(f"Failed to pack data to exactly {self.num_images_expected} images, yield a data sample with {buffer_list[0]['pixel_values'].size(0)} images.")
                if self.strict_mode:
...
def packed_collate_fn(
    ...
    if len(features) < micro_num and WARNING_CNT["micro_num_warning"] < 5:
        logger.warning(f"{len(features)=} > {micro_num=}, the features will be padded to satisfy micro_num requirement")
