...
class MultiheadAttention(Module):
    ...
    def forward(
        self, query: Tensor, key: Tensor, value: Tensor, key_padding_mask: Optional[Tensor] = None, need_weights: bool = True, attn_mask: Optional[Tensor] = None, average_attn_weights: bool = True, decoder_model=None
...
def _scaled_dot_product_attention(q: Tensor, k: Tensor, v: Tensor, attn_mask: Optional[Tensor] = None, dropout_p: float = 0.0, decoder_model=None) -> Tuple[Tensor, Tensor]:
    r"""
    ...
    if decoder_model is not None:
        print("begin", decoder_model.origin_idx.shape)
        retain_ind = decoder_model.eliminate_token(attn2, decoder_model.origin_idx)
        print("end", decoder_model.retain_ind.shape)
...
def multi_head_attention_forward(
    ...
    #
    # attn_output, attn_output_weights = _scaled_dot_product_attention(q, k, v, attn_mask, dropout_p, decoder_model)
    B, Nt, E = q.shape
    q = q / math.sqrt(E)
    # (B, Nt, E) x (B, E, Ns) -> (B, Nt, Ns)
    attn = torch.bmm(q, k.transpose(-2, -1))
    ...
    if decoder_model is not None:
        with torch.no_grad():
            attn2 = attn_output_weights.view(bsz, num_heads, tgt_len, src_len)
            attn2 = attn2.sum(dim=1) / num_heads
            if not is_batched:
                attn2 = attn2.squeeze(0)
            attn2 = attn2.mean(-1)
        # tmp_return_list_del.append(attn2)
        # 获取需要保留的token index   attn2 [8, 784] decoder_model.origin_idx.shape [8, 785]
        retain_ind = decoder_model.eliminate_token(attn2, decoder_model.origin_idx)  # [8, 783]
        # 根据retain_ind 修改tgt_key_padding_mask
        decoder_model.tgt_key_padding_mask = decoder_model.get_from_idx_vl(decoder_model.tgt_key_padding_mask, retain_ind)
        # 根据retain_ind origin_idx
        decoder_model.origin_idx = decoder_model.get_from_idx_vl(decoder_model.origin_idx, retain_ind)
        # [8,783] -> [783, 8, 768]
        retain_ind = retain_ind.permute(1, 0).unsqueeze(-1).expand(-1, -1, decoder_model.query_pos.shape[-1]).contiguous()
        # 更新解码器的ids和位置编码
        decoder_model.query_pos = decoder_model.get_from_idx_vl(decoder_model.query_pos, retain_ind)
        decoder_model.retain_ind = retain_ind
