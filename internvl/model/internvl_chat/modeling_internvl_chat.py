...
class InternVLChatModel(PreTrainedModel):
    ...
    _no_split_modules = ["InternVisionModel", "LlamaDecoderLayer", "InternLM2DecoderLayer", "Phi3DecoderLayer", "Qwen2DecoderLayer"]
    ...
    def wrap_llm_lora(self, r=128, lora_alpha=256, lora_dropout=0.05):
        ...
        if self.llm_arch_name == "InternLM2ForCausalLM":
            ...
            target_modules = ["self_attn.q_proj", "self_attn.k_proj", "self_attn.v_proj", "self_attn.o_proj", "mlp.gate_proj", "mlp.down_proj", "mlp.up_proj"]
        else:
    ...
    def forward(
        ...
        return_vit_feature: Optional[bool] = False,
        ...
        vit_embeds, vit_feature = self.extract_feature(pixel_values)
    ...
    def extract_feature(self, pixel_values):
        ...
        vit_feature = vit_embeds.clone()
        h = w = int(vit_embeds.shape[1] ** 0.5)
        vit_embeds = vit_embeds.reshape(vit_embeds.shape[0], h, w, -1)
        vit_embeds = self.pixel_shuffle(vit_embeds, scale_factor=self.downsample_ratio)
        vit_embeds = vit_embeds.reshape(vit_embeds.shape[0], -1, vit_embeds.shape[-1])
        vit_embeds = self.mlp1(vit_embeds)
        return vit_embeds, vit_feature
    ...
    def chat(self, tokenizer, pixel_values, question, generation_config, history=None, return_history=False, num_patches_list=None, IMG_START_TOKEN="<img>", IMG_END_TOKEN="</img>", IMG_CONTEXT_TOKEN="<IMG_CONTEXT>", verbose=False):
        ...
        generation_config["eos_token_id"] = eos_token_id
        generation_output = self.generate(pixel_values=pixel_values, input_ids=input_ids, attention_mask=attention_mask, **generation_config)
        response = tokenizer.batch_decode(generation_output[0], skip_special_tokens=False)[0]
